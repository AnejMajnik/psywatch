...
int main() {
  ...
  // RAM
  std::string ram_usage = get_ram_usage();
  ...
  auto gpu_data_component =
      GpuData(gpu_temp, gpu_usage, vram_usage, power_draw);
  auto graph_gpu_component = GpuUsageGraph(gpu_usage_log);
  auto graph_cpu_component = CpuUsageGraph(cpu_usage_log);
  auto ram_data_component = RamData(ram_usage);
